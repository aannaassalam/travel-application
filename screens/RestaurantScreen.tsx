...
export default function RestaurantScreen() {
  const { slug } = useRoute<RouteProp<RootStackParamList, "Restaurant">>().params;
    ...
    <View style={styles.screen}>
      ...
      >
        ...
        <View style={styles.body}>
          ...
          <Reveal index={1} delay={240} duration={600} style={styles.metaRow}>
            {r.rating ? (
          ...
          {r.deliveryZones.length ? (
            <Reveal index={2} delay={240} duration={600}>
          ...
          {sections.map((group, gi) => (
            <View key={group.section} style={styles.section}>
              <Reveal index={3 + gi} delay={240} duration={600}>
              ...
              <View style={{ gap: space[3] }}>
                {group.items.map((item, i) => (
                  <Reveal key={item.id} index={i} delay={340} duration={600}>
      ...
      <CartBar bottomOffset={0} insetBottom />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  skeletonHero: { height: 260, borderRadius: 0 },
